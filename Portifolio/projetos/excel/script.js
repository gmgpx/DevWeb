...
function renderTable() {
  ...
  data.forEach(user => {
    ...
    row.innerHTML = `<td>${user.id}</td><td>${user.username}</td><td>${user.password}</td><td class="action-buttons"><button onclick="edit(${user.id})">Edit</button><button onclick="remove(${user.id})">Delete</button></td>`;
  });
}
